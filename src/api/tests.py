...
class TestUploadReport:
    ...
    def test_upload_report_rejects_concatenated_payload(self, test_license_key, sample_lynis_report):
        """Test concatenated Lynis payloads are rejected as malformed."""
        client = Client()
        url = reverse('upload_report')

        malformed_payload = f"{sample_lynis_report}\n{sample_lynis_report}"
        response = client.post(url, {
            'licensekey': test_license_key.licensekey,
            'hostid': 'concat-host-id-1',
            'hostid2': 'concat-host-id-2',
            'data': malformed_payload,
        })

        assert response.status_code == 400
        assert response.content == b'Invalid report data'
        assert not Device.objects.filter(hostid='concat-host-id-1', hostid2='concat-host-id-2').exists()

    def test_upload_report_does_not_clear_hostname_when_missing(self, test_device, sample_lynis_report):
        """Test missing hostname in a valid update does not overwrite existing hostname."""
        client = Client()
        url = reverse('upload_report')

        test_device.hostname = 'keycloak'
        test_device.save(update_fields=['hostname'])

        payload_without_hostname = '\n'.join(
            line for line in sample_lynis_report.splitlines() if not line.startswith('hostname=')
        )
        ...
        response = client.post(url, {
            ...
            'hostid': test_device.hostid,
            'hostid2': test_device.hostid2,
            'data': payload_without_hostname,
        })

        assert response.status_code == 200
        test_device.refresh_from_db()
        assert test_device.hostname == 'keycloak'
